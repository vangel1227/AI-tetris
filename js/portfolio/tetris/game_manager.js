/* 
Generates games canvas, uses innerHTML to change text content of buttons & game windows 
Button click events, graphics, animation gam controls
...
function GameManager(){
    ...
    var isAiActive = false; // AI true means automatically in AI mode
    ...
    aiButton.onclick = function(){
        if (isAiActive){ // Turning AI off
            isAiActive = false; 
            aiButton.classList.remove('btn-info');
            aiButton.classList.add('btn-warning');
            aiButton.innerHTML = 'Run AI';
        }else{ // Turning AI on
            isAiActive = true;
            aiButton.classList.remove('btn-warning');
    ...
    startButton.onclick = function(){
        if (!start) {
            start = true;
            gravityTimer = new Timer(onGravityTimerTick, 100);
            startTurn();
            startButton.remove();
        }
    }
